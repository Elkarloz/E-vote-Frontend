<template>
    ...
    <div class="card">
        <div class="card-header">
            <div class="col-md-12">
            <h3 class="text-center">CONSULTAR PERSONA</h3>
            </div>
        </div>
    </div>
    <div class="card">
          ...
          <div class="modal-content">
            ...
            <form @submit.prevent="ActualizarUsuario()">
              ...
              <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-danger" data-toggle="modal" data-target="#modalEliminar" data-dismiss="modal" >Eliminar</button>
                <button type="submit" class="btn btn-success" data-toggle="modal" @click="cerrarModal()">Actualizar</button>
              </div>
            </form>
          </div>
        </div>
...
fetch(this.ruta + `/${DocPer}` + `/${NomPer}` + `/${ApePer}` + `/${Ficha}` + `/${Jornada}` + `/${undefined}`, {
        ...
        fetch(this.ruta + `/${DocPer}` + `/${NomPer}` + `/${ApePer}` + `/${Ficha}` + `/${Jornada}` + `/${Programa}`, {
        method: "GET",
        mode: "cors",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
